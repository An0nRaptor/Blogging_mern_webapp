...
const CommentCard = ({ leftVal, index, commentData }) => {
  ...
  const loadReplies=({skip=0, currentIndex = index})=>{
    ...
    if(commentsArr[currentIndex].children.length){
      ...
      axios.post(import.meta.env.VITE_HOST + "/get-replies",{_id:commentsArr[currentIndex]._id,skip})
  ...
  const handleDeleteComment =(e)=>{
      ...
      axios.post(import.meta.env.VITE_HOST + '/delete-comment',{_id},{
  ...
  return (
    ...
    <div className="w-full" style={{ paddingLeft: `${leftVal * 10}px` }}>
      <div className="my-5 p-6 rounded-md border border-grey">
        ...
        {
          commentData.isReplyLoaded ? <button className="text-dark p-2 px-3 hover:bg-grey/30 rounded-md flex items-center gap-2" onClick={handleHideComment}>
          <i className="fi fi-rs-comment-dots"></i>Hide Reply
          </button> : <button onClick={loadReplies} className="text-dark p-2 px-3 hover:bg-grey/30 rounded-md flex items-center gap-2">
          <i className="fi fi-rs-comment-dots"></i>{children.length} Reply 
          </button> 
        }
          <button onClick={handleClickReply} className="underline">Reply</button>
            
            {
              username == commented_by_username || username == blog_author ? <button className="p-2 px-3 rounded-md border border-grey ml-auto hover:bg-red/30 hover:text-red flex items-center" onClick={handleDeleteComment}>
