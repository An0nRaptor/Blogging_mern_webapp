...
const NotificationCard = ({data,index,notificationState}) => {
    ...
    let {seen,reply,type,createdAt,comment,replied_on_comment,user,user:{personal_info:{fullName,username,profile_img}},blog:{_id,blog_id,title},_id:notification_id} = data;
    ...
    const handleDelete=(comment_id,type,target)=>{
        ...
        axios.post(import.meta.env.VITE_HOST + "/delete-comment",{
                ...
                <div className="flex gap-5 mb-3">
                        ...
                        <h1 className="font-medium text-xl text-dark-grey">
...

                            {
                                ...
                                </div> : <Link to={`/blogs/${blog_id}`} className="font-medium text-dark-grey hover:underline line-clamp-1">{`"${title}"`}</Link>
                            }
...

                {
                    reply ? <div className="ml-20 p-5 bg-grey mt-5 rounded-md">
                            <div className="flex gap-3 mb-3 ">
                                    ...
                                    <div>
                                        <h1 className="font-medium text-xl text-dark-grey">
                                            <Link className="mx-1 text-black underline" to={`/user/${author_username}`}>@{username}</Link>
